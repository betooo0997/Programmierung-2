...
namespace Game
{
    class TileManager
    {
        ...
        public void Draw(RenderWindow window, Vector2f tileMapPosition)
        {
            ...
            {
                if ((xCoord + tileMapPositionXNormalized) >= 0 && (xCoord + tileMapPositionXNormalized) < GetNumberColumns() &&
                    (yCoord + tileMapPositionYNormalized) >= 0 && (yCoord + tileMapPositionYNormalized) < GetNumberRows())
                {
                    tileSheet.Position = new Vector2f(((int)(((xCoord + tileMapPositionXNormalized) * tileArrayCreation.GetTileSize()) + tileMapPosition.X)),
                        (int)((((yCoord + tileMapPositionYNormalized) * tileArrayCreation.GetTileSize()) + tileMapPosition.Y)));
                    tileSheet.TextureRect = TileSourceDeterminat0r(tileArrayCreation.GetTilezArray()[xCoord + tileMapPositionXNormalized, tileMapPositionYNormalized + yCoord]);
